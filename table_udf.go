...
void table_udf_bind(duckdb_bind_info info);
void table_udf_init(duckdb_init_info info);
void table_udf_callback(duckdb_function_info, duckdb_data_chunk);
void table_udf_destroy_data(void *);

typedef void (*table_udf_init_t)(duckdb_function_info);
typedef void (*table_udf_bind_t)(duckdb_function_info);
typedef void (*table_udf_callback_t)(duckdb_function_info, duckdb_data_chunk);
*/
import "C"
...
//export table_udf_bind
func table_udf_bind(info C.duckdb_bind_info) {
...
func table_udf_callback(info C.duckdb_function_info, output C.duckdb_data_chunk) {
...
func RegisterTableUDF(c *sql.Conn, functionName string, function TableFunctionProvider) error {
	...
	err := c.Raw(func(anyConn any) error {
		...
		C.duckdb_table_function_set_name(tableFunction, cFunctionName)
		C.duckdb_table_function_set_bind(tableFunction, C.table_udf_bind_t(C.table_udf_bind))
		C.duckdb_table_function_set_init(tableFunction, C.table_udf_init_t(C.table_udf_init))
		C.duckdb_table_function_set_function(tableFunction, C.table_udf_callback_t(C.table_udf_callback))
		C.duckdb_table_function_set_extra_info(tableFunction, unsafe.Pointer(&extraInfoHandle), C.duckdb_delete_callback_t(C.table_udf_destroy_data))
		...
		state := C.duckdb_register_table_function(driverConn.duckdbCon, tableFunction)
